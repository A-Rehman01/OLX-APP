...
import { productdata, getProductbyFilter } from '../../Reducer/ProductSlice';
...
export function Cards() {
  ...
  const filterdata = useSelector(getProductbyFilter);
  ...
  return (
    <div className={style.CardsCantainer}>
      ...
      >
        {!filterdata.filterProduct.length
          ? data?.productlist?.map((obj) => {
              return (
                <Grid key={obj._id} item xs={11} sm={3}>
          ...
          : filterdata?.filterProduct?.map((obj) => {
              return (
                <Grid key={obj._id} item xs={11} sm={3}>
                  <Link to={`product/${obj._id}`}>
                    <div className={style.card}>
